...
function placeXOrO(squareNumber) {
    //This condition ensures a square hasn't been selected already, the .some method is used to check each element of the selectSquare array to see if it contains the same number clicked on.
    if(!selectedSquares.some(element => element.includes(squareNumber))) {
        //This bariable retireves the HTML element id that was clicked.
        ...
        if (activePlayer === 'O') {
            ...
            disableClick();
            //this function waits 1 sec before the computer places an img and enables click
...
function checkWinConditions() {
    ...
    //This condition checks for a tie, If none of the above conditions are net and 9 squres are selected the code executes.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    let x1 = coordX1,
        ...
        y2 = coordY2,
        //This varaible stores temp x axis data we update in our animation loop.
        x = x1,
...
function animateLineDrawing() {
    ...
    //This condition is similar to the one above for the 6,4,2 condition.
    if (x1 <= x2 && y1 >= y2) {
        if (x < x2) { x += 10; }
        if (y > y2) { y-= 10; }
        if (x >= x2 && y <= y2) {cancelAnimationFrame(animationLoop); }
    ...
    animateLineDrawing();
    //This line waits one second then clears canvas resets game an allows clicking again.
